...
def run_training():
    ...
    def collate_batch_with_lengths(batch):
        ...
        for (_text, _label) in batch:
            ...
            lengths_list.append(len(processed_text))
    ...
    train_loader = DataLoader(
        ...
        shuffle=config.SHUFFLE_DATA, collate_fn=collate_batch_with_lengths,
        num_workers=config.NUM_WORKERS, pin_memory=pin_memory
    )
    val_loader = DataLoader(
        dataset=val_dataset, batch_size=config.BATCH_SIZE,
        shuffle=False, collate_fn=collate_batch_with_lengths,
        num_workers=config.NUM_WORKERS, pin_memory=pin_memory
    )
    ...
    print("\nStarting training...")
    trained_model, history_df = engine.trainer_with_lengths(
        model=model,
