...
def main():
    ...
    # Allow overriding specific Transformer config options if needed, but MODEL_TYPE is fixed
    ...
    # Add other relevant overrides as needed
    ...
    if args.batch_size and args.batch_size != config.TRAIN_BATCH_SIZE:
         ...
         # Optionally override valid batch size too, or keep it separate
         # config.VALID_BATCH_SIZE = args.batch_size * 2 # Example adjustment
         config_overridden = True
    ...
    print(f"Artifacts will be saved in: {config.MODEL_TYPE_ARTIFACTS_DIR}") # Always artifacts/Transformer

    # Ensure artifact directories exist (config.py does this, but check again)
    try:
        os.makedirs(config.MODEL_TYPE_ARTIFACTS_DIR, exist_ok=True)
        os.makedirs(config.MODEL_SAVE_DIR, exist_ok=True)
        if os.path.dirname(config.LABEL_MAP_PATH): # Ensure base artifacts dir exists
    ...
    # --- Execute Training ---
    try:
        train.run_training_pipeline() # Calls the pipeline using the current config state
        ...
        sys.exit(1)
    except ImportError as e: # Catch library import errors during pipeline run
