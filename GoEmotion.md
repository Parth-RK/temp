Okay, let's generate the complete code for each file, incorporating the handling of a TSV dataset with comma-separated multi-labels, the improved emoji cleaning, and the necessary multi-label adaptations throughout the pipeline.

This assumes you have:
1.  GoEmotions data in TSV files (e.g., `train.tsv`, `validation.tsv`, `test.tsv`) in the `data` directory.
2.  These TSV files have no header row (`HAS_HEADER = False`).
3.  The 0th column contains the text (`TEXT_COLUMN_INDEX = 0`).
4.  The 1st column contains comma-separated label strings (e.g., "anger,disgust") (`LABEL_COLUMN_INDEX = 1`).
5.  The `new_label_map.json` file exists and contains the 28 GoEmotions labels and their integer mappings. This file will be the definitive source for label integers and names.
...
nltk>=3.6.0
regex>=2021.8.0
emoji>=2.0.0 # Added emoji library
```
...
VALID_FILE = "validation.tsv" # Example TSV validation file name
...
--- START OF FILE new_label_map.json ---
```json
{
    "0": "admiration",
    "1": "amusement",
...
```
--- END OF FILE new_label_map.json ---

--- START OF FILE ekman_mapping.json ---
```json
{
...
```
--- END OF FILE ekman_mapping.json ---

--- START OF FILE sentiment_mapping.json ---
```json
{
...
```
--- END OF FILE sentiment_mapping.json ---

**To use this code:**

1.  Save each block into its corresponding file name.
2.  Ensure your GoEmotions data (prepared into `train.tsv`, `validation.tsv`, `test.tsv` with the specified 0th and 1st columns and *no header*) is placed in a directory named `data`.
3.  Place the `new_label_map.json` file in the main project directory (where `main.py` is) or update `config.LABEL_MAP_PATH` to point to its location.
4.  Run `pip install -r requirements.txt` to install the necessary libraries, including `emoji`.
5.  Modify `config.py` if your file names, columns, or header status are different from the assumptions made above. Double-check `METRIC_FOR_BEST_MODEL` and `PREDICTION_THRESHOLD`.
6.  Run the training: `python main.py`.
7.  After training, run the app: `python app.py`.

This setup fully adapts your pipeline for multi-label classification on the GoEmotions dataset, incorporates explicit emoji handling, and provides the infrastructure to load and use the trained model with a Gradio interface.
